...
/** Ширина листа А4 (210 мм) в пикселях при 96 dpi */
const A4_PX = 794;

/**
 * Лист А4. Zoom "fit" подгоняет лист под ширину панели,
 * числовые значения показывают натуральную величину в процентах.
 */
function Scaled({ children, cls, zoom = "fit" }: { children: ReactNode; cls?: string; zoom?: number | "fit" }) {
  const ref = useRef<HTMLDivElement>(null);
  const host = useRef<HTMLDivElement>(null);
  const [h, setH] = useState(1120);
  const [w, setW] = useState(A4_PX);

  useLayoutEffect(() => {
    const el = ref.current;
    if (el) setH(el.offsetHeight || 1120);
    if (host.current) setW(host.current.offsetWidth || A4_PX);
  ...
  const fit = Math.min(1, (w - 8) / A4_PX);
  const s = zoom === "fit" ? fit : zoom;

  return (
    <div ref={host} className={"w-full " + (cls ?? "")}>
      {s < 1 ? (
        <div className="sheet-wrap relative" style={{ width: A4_PX * s, height: h * s, margin: "0 auto" }}>
          <div
            ref={ref}
            className="print-scale absolute left-0 top-0 origin-top-left shadow-[0_10px_36px_rgba(15,23,42,0.18)]"
            style={{ width: A4_PX, transform: `scale(${s})` }}
          >
            {children}
          </div>
        </div>
      ) : (
        // при 100% включаем прокрутку, чтобы лист не обрезался
        <div className="overflow-x-auto">
          <div
            ref={ref}
            className="print-scale origin-top-left shadow-[0_10px_36px_rgba(15,23,42,0.18)]"
            style={{ width: A4_PX }}
          >
            {children}
          </div>
    ...
    <div className="min-h-screen bg-slate-100 text-slate-900 print:bg-white">
      ...
      <main className="mx-auto grid max-w-[1700px] grid-cols-1 gap-5 p-4 xl:grid-cols-[minmax(430px,1fr)_minmax(0,1.05fr)]">
        ...
        <div className={`${tab === "preview" ? "" : "hidden xl:block"}`}>
          <div className="no-print mb-3 flex flex-wrap items-center gap-2 rounded-2xl border border-slate-200 bg-white p-2.5 shadow-sm">
            ...
            <span className="ml-2 hidden h-5 w-px bg-slate-200 sm:block" />

            <div className="flex items-center gap-1 rounded-lg bg-slate-100 p-0.5">
              {([["fit", "по ширине"], [1, "100%"], [1.4, "140%"]] as const).map(([z, l]) => (
                <button
                  key={String(z)}
                  onClick={() => setZoom(z as number | "fit")}
                  className={`rounded-md px-2 py-1 text-[11px] font-semibold transition ${String(zoom) === String(z) ? "bg-white text-indigo-600 shadow-sm" : "text-slate-500 hover:text-slate-700"}`}
                >
                  {l}
                </button>
              ))}
          ...
          <div className="print-root rounded-3xl bg-slate-200/70 p-3">
            {page === 1 ? (
              ...
              <>
                ...
                <Scaled cls="print-break" zoom={zoom}>
                  <div className="frm">
                    <Sheet2 d={s} />
                  </div>
                </Scaled>
